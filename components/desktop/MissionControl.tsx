...
export function MissionControl() {
  const { missionControlOpen, windows, toggleMissionControl, bringToFront } = useDesktopStore();
    ...
    <AnimatePresence>
      {missionControlOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-md flex flex-col pt-12 pb-8 px-8"
          onClick={(e) => {
        ...
        >
          <div className="flex justify-between items-center mb-8 text-white px-4">
            <h2 className="text-2xl font-light tracking-tight">Mission Control</h2>
            <button
            ...
            {windows.filter(w => w.isOpen).map((window) => (
              ...
              >
                ...
                <div className="absolute top-0 inset-x-0 h-8 bg-zinc-100 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-700 flex items-center px-4 gap-2">
                  ...
                  <span className="text-xs text-zinc-500 font-medium truncate">{window.title}</span>
                </div>
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="font-semibold text-zinc-700 dark:text-zinc-200 mt-2">{window.title}</span>
                </div>
              </motion.button>
            ))}
